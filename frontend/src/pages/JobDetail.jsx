...
export default function JobDetail({ user }) {
  ...
  useEffect(() => {
    loadJob();
    if (user?.role === "freelancer") checkUserApplication();
    if (user?.role === "client") loadApplicationsForClient();
  }, [id, user]);
  ...
  const checkUserApplication = async () => {
    try {
      const res = await jobApplicationsAPI.list({
        job: id,
        freelancer: user.id,
      });
      setUserApplication(res.data.results?.[0] || null);
    } catch {
